...
public final class ExportHandler {
	...
	public void execute() {
		...
		if (pages == null) {
			MessageDialog.openError(shell, Messages.NoPagesSelected,
					Messages.SelectPagesToExport);
			return;
		}
		...
		if (location != null) {
			save(pages, location);
			String pagesString = pages.size() == 1 ? Messages.Page
					: Messages.Pages;
			MessageDialog
					.openInformation(
							shell,
							Messages.Export,
							String.format(
									"%s "	+ pagesString + " " + Messages.ExportedTo + " %s", pages.size(), location)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		}
	}
