...
object User {
  ...
  def fromMap(m: java.util.Map[String, AnyRef]): User = {
    ...
    u.upvotes = map("upvotes").toString.trim.toInt
    u.upvoted = map("upvoted").toString.trim.toInt
    u.downvotes = map("downvotes").toString.trim.toInt
    u.downvoted = map("downvoted").toString.trim.toInt
